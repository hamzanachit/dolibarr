...
/**
 *	\file       releve/releveindex.php
 *	\ingroup    releve
 *	\brief      Home page of releve top menu
 */
...
//var_dump($search_montant_ht); die;
if ($search_montant_ht != '') {
	$sql .= natural_search('f.total_ht', $search_montant_ht, 0);
}
...
if (!$sall) {
	$sql .= ' GROUP BY f.fk_soc, f.type, f.note_private, f.note_public, f.increment, f.fk_mode_reglement, f.fk_cond_reglement, f.total_ht, f.total_tva, f.total_ttc,';
...
if ($resql) {
	...
	print '<div class="tmenucenter1"><div class="menulogocontainer1">';
	print '<img class="mycompany" title="Allez dans Accueil - Configuration - Société/institution pour changer le  logo ou aller dans Accueil - Configuration - Affichage pour le cacher." alt="" src="/fap/htdocs/theme/dolibarr_512x512_white.png" style="max-width: 260px">';
	print '</div>';
	...
	print '<input type="hidden" name="search_status" value="1">';
	...
	print '<table class="tagtable liste'.($moreforfilter ? " listwithfilterbefore" : " styled-table").'">'."\n";
	...
	//var_dump($arrayfields['f.total_ht']['checked']);
	...
	if (!empty($arrayfields['f.fk_statut']['checked'])) {
		...
		print $form->selectarray('search_status', $liststatus, 1, 1, 0, 0, '', 0, 0, 0, '', '', 1);
		print '</td>';
	}
	// Action column
	...
	if (!empty($arrayfields['f.fk_statut']['checked'])) {
		print_liste_field_titre($arrayfields['f.fk_statut']['label'], $_SERVER["PHP_SELF"], "f.fk_statut,f.paye,f.type,dynamount_payed", "", $param, 'class="center"', $sortfield, $sortorder);
	}
	if(!$printing){
	...
	if ($num > 0) {
		...
		while ($i < min($num, $limit)) {
			...
			if($i==0 && $printing){
				print '<div class="titre_releve" style="font-size:18px;font-weight: 600;">Relevé de compte client -'.$obj->name.'</div>';
				
				print "<p style='font-weight: 600 !important;'>Madame, Monsieur</p>
				<p>Après examen de votre compte en nos livres et sauf erreur ou omission de notre part vous nous restez
				redevable de la somme ci-dessus qui se décompose ainsi :</p>";
			}
			$marginInfo = array();
			if (!empty($conf->margin->enabled)) {
		...
		if (isset($totalarray['pos'])) {
			print '<tr class="liste_total style="font-weight: bold;color: black;font-size:14px;">';
			$i = 0;
			while ($i < $totalarray['nbfield']) {
				$i++;
				//var_dump(price($totalarray['val'][$totalarray['pos'][$i]]));
				if (!empty($totalarray['pos'][$i])) {
					print '<td class="right" style="font-weight: bold;color: black;font-size:14px;">'.price($totalarray['val'][$totalarray['pos'][$i]]).'</td>';
				} else {
					if ($i == 1) {
	...
	print "</table>\n";
	print '</div>';
	//var_dump($totalarray['val']['rtp']);
	if( $printing){
		print "<p>Persuadés qu'il ne s'agit que d'un simple oubli de votre part, nous vous prions de bien vouloir nous en faire
		parvenir le réglement au plus tôt.</p>
		<p>Si toutefois vous avez effectué ce réglement récemment, veuillez ne pas tenir compte de cette lettre.</p>
