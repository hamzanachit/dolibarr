...
function nombreEnLettres($nombre)
{
    ...
    list($integerPart, $decimalPart) = explode('.', $nombre . '.0');
    ...
    if ($integerPart == 0) {
        ...
        $units = ['', '', 'deux', 'trois', 'quatre', 'cinq', 'six', 'sept', 'huit', 'neuf'];
        $teens = ['','','dix', 'onze', 'douze', 'treize', 'quatorze', 'quinze', 'seize', 'dix-sept', 'dix-huit', 'dix-neuf'];
        $tens = ['','','vingt', 'trente', 'quarante', 'cinquante', 'soixante', 'soixante-dix', 'quatre-vingt', 'quatre-vingt-dix'];

        $thousands = ['', ' mille ', 'million', 'milliard', 'billion', 'billiard', 'trillion', 'trilliard'];

        if ($integerPart < 1000) {
            // If the integer part is less than 1000, directly process it without 'mille'
            $hundreds = floor($integerPart / 100);
            $remainder = $integerPart % 100;

            if ($hundreds != 0) {
                if ($hundreds == 1 && $convert !== '') {
                    // Add 'et' before 'mille' if not the first segment
                    $convert .= ' et ';
                }
                if ($hundreds == 1) {
                    // If the hundreds place is 1, append 'mille' instead of 'cent'
                    $convert .= ' mille ';
                } else {
                    $convert .= $units[$hundreds] . ' cents';
                }
                if ($remainder != 0) {
                    $convert .= ' ';
                }
            ...
            if ($remainder != 0) {
                if ($remainder < 10 && $hundreds != 1) {
                    $convert .= $units[$remainder];
                } elseif ($remainder < 20) {
                    $convert .= $teens[$remainder - 10];
                } else {
                    $tensIndex = floor($remainder / 10);
                    $unitsIndex = $remainder % 10;
                    if ($unitsIndex != 0) {
                        if ($tensIndex == 7 || $tensIndex == 9) {
                            $tensWord = $tens[$tensIndex - 1];
                            $tensWord = rtrim($tensWord, 't');
                            $convert .= $tensWord . '-' . $units[$unitsIndex];
                        } else {
                            $convert .= $tens[$tensIndex] . '-' . $units[$unitsIndex];
                        }
                    } else {
                        $convert .= $tens[$tensIndex];
                    }
                }
            }
        } else {
            // Process the integer part with thousands
            $nbSegments = floor((strlen($integerPart) - 1) / 3) + 1;
            $integerPart = str_pad($integerPart, $nbSegments * 3, '0', STR_PAD_LEFT);

            $Segments = str_split($integerPart, 3);

            for ($i = 0; $i < $nbSegments; $i++) {
                $nb = (int)$Segments[$i];
                if ($nb != 0) {
                    $hundreds = floor($nb / 100);
                    $remainder = $nb % 100;
                    ...
                    if ($nbSegments - $i - 1 > 0) {
                        $convert .= ' ' . $thousands[$nbSegments - $i - 1];
                        if ($i < $nbSegments - 2) {
                            $convert .= ' ';
                        }
                    }
        ...
        $convert .= ' Dirhams';  // Add Dirhams at the end
    }

    // Process the decimal part
    if ($decimalPart != 0) {
        // Add the word 'et' before the decimal part
        $convert .= ' et ';

        // Process the decimal part without including 'mille'
        $decimalUnits = ['', 'un', 'deux', 'trois', 'quatre', 'cinq', 'six', 'sept', 'huit', 'neuf'];
        $decimalTens = ['','dix', 'vingt', 'trente', 'quarante', 'cinquante', 'soixante', 'soixante-dix', 'quatre-vingt', 'quatre-vingt-dix'];

        if ($decimalPart < 10) {
            $convert .= $decimalUnits[$decimalPart];
        } elseif ($decimalPart < 20) {
            $convert .= 'dix-' . $decimalUnits[$decimalPart - 10];
        } else {
            $tensIndex = floor($decimalPart / 10);
            $unitsIndex = $decimalPart % 10;

            if ($unitsIndex != 0) {
                if ($tensIndex == 7 || $tensIndex == 9) {
                    $tensWord = $decimalTens[$tensIndex - 1];
                    $tensWord = rtrim($tensWord, 't');
                    $convert .= $tensWord . '-' . $decimalUnits[$unitsIndex];
                } else {
                    $convert .= $decimalTens[$tensIndex] . '-' . $decimalUnits[$unitsIndex];
                }
            } else {
                $convert .= $decimalTens[$tensIndex];
            }
        }
...
//-                         $convert .= ' cent';
//-                     } else {
//-                         $convert .= $units[$hundreds] . ' cent';
//-                     }
//-                 }

//-                 if ($remainder < 20 && $remainder > 0) {
//-                     $convert .= ($convert ? ' ' : '') . ($remainder < 10 ? $units[$remainder] : $teens[$remainder - 10]);
//-                 } elseif ($remainder >= 20) {
//-                     $tensIndex = floor($remainder / 10);
//-                     $unitsIndex = $remainder % 10;
//-                     $convert .= ($convert ? ' ' : '') . $tens[$tensIndex];
//-                     if ($unitsIndex > 0) {
//-                         $convert .= '-' . $units[$unitsIndex];
//-                     }
//-                 }
...
//-         if ($decimalPart < 20) {
//-             $convert .= $decimalPart < 10 ? $units[$decimalPart] : $teens[$decimalPart - 10];
//-         } else {
//-             $tensIndex = floor($decimalPart / 10);
//-             $unitsIndex = $decimalPart % 10;
//-             $convert .= $tens[$tensIndex];
