 ...
 */
 function pdf_getlinedesc($object, $i, $outputlangs, $hideref, $hidedesc = 0, $issupplierline = 0, $separateRef = false)
{
    global $db, $conf, $langs;

    // Existing logic for fetching label, description, reference, etc.
    ...
    // Logic for building the reference string
    if (empty($hideref) && $hideref == 100) {
        $ref_prodserv = $prodser->ref; // Default to local ref
        if ($issupplierline && $ref_supplier) {
            $ref_prodserv .= ' (' . $outputlangs->transnoentitiesnoconv("SupplierRef") . ' ' . $ref_supplier . ')';
        }

        // Customer-specific reference
        if (!empty($conf->global->PRODUIT_CUSTOMER_PRICES) && !empty($productCustomerPrice->ref_customer)) {
            $ref_prodserv .= ' (' . $outputlangs->transnoentitiesnoconv('RefCustomer') . ' ' . $productCustomerPrice->ref_customer . ')';
        }
    }

    // Bold formatting for the reference if required
    if (!empty($ref_prodserv) && !empty($conf->global->PDF_BOLD_PRODUCT_REF_AND_PERIOD)) {
        $ref_prodserv = '<b>' . $ref_prodserv . '</b>';
    }

    // Combine reference and label
    if ($separateRef) {
        // If $separateRef is true, put reference on a new line or separate output
        $libelleproduitservice = $ref_prodserv . "__N__" . $prefix_prodserv . $label;
    } else {
        // If not separating, include reference as part of the same line
        $libelleproduitservice = $prefix_prodserv . $ref_prodserv . $label;
    }

    // Continue with the existing logic for handling descriptions, notes, batches, etc.
    ...
    // Replace newline placeholders with HTML line breaks or plain text line breaks
    ...
    // Convert any remaining HTML entities to display properly
    $libelleproduitservice = dol_htmlentitiesbr($libelleproduitservice, 1);

    return ucwords(strtolower($libelleproduitservice));
}
 ...
 * 	@return	string
 */
function pdf_getlinebl_ref($object, $i, $outputlangs, $hidedetails = 0)
{
	//var_dump($object->lines[$i]->ref);exit;
	global $hookmanager;
	...
	if (is_object($hookmanager)) {   // Old code is commented on preceding line. Reproduct this test in the pdf_xxx function if you don't want your hook to run
		$special_code = $object->lines[$i]->special_code;
		if (!empty($object->lines[$i]->bl_ref)) {
			$special_code = $object->getSpecialCode($object->lines[$i]->bl_ref);
		}
		$parameters = array('i'=>$i, 'outputlangs'=>$outputlangs, 'hidedetails'=>$hidedetails, 'special_code'=>$special_code);
		$action = '';
		$reshook = $hookmanager->executeHooks('pdf_getlinebl_ref', $parameters, $object, $action); // Note that $action and $object may have been modified by some hooks
		$result .= $hookmanager->resPrint;
	}
	if (empty($reshook)) {
		$result .= dol_htmlentitiesbr($object->lines[$i]->bl_ref);
	}
	return $result;
}


function pdf_getproduct_ref($object, $i, $outputlangs, $hidedetails = 0)
{
	...
	if (is_object($hookmanager)) {   // Old code is commented on preceding line. Reproduct this test in the pdf_xxx function if you don't want your hook to run
		...
		if (!empty($object->lines[$i]->ref)) {
			$special_code = $object->getSpecialCode($object->lines[$i]->ref);
		}
		$parameters = array('i'=>$i, 'outputlangs'=>$outputlangs, 'hidedetails'=>$hidedetails, 'special_code'=>$special_code);
		$action = '';
		$reshook = $hookmanager->executeHooks('pdf_getproduct_ref', $parameters, $object, $action); // Note that $action and $object may have been modified by some hooks
		$result .= $hookmanager->resPrint;
	}
	if (empty($reshook)) {
		$result .= dol_htmlentitiesbr($object->lines[$i]->ref);
	}
	return $result;
}
