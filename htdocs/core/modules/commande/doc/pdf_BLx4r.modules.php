 ...
 */
class pdf_BLx4r extends ModelePDFCommandes
{
	/**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "BLx4r";
		$this->description = $langs->trans('x4rBL');
		...
		if ($conf->commande->multidir_output[$conf->entity]) {
			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					//var_dump($this->posxpicture-$curX-$progress_width+$this->posxup-$this->posxtva+4);die;
					$pdf->startTransaction();
					pdf_writelinedesc($pdf,$object,$i,$outputlangs,$this->posxpicture-$curX-$progress_width+37,3,$curX,$curY,$hideref,$hidedesc);
					...
					//$blref = pdf_writeLinkedObjects($pdf,$object,$i,$outputlangs,$this->posxpicture-$curX-$progress_width+37,3,$curX,$curY,$hideref,$hidedesc)
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "PdfOrderTitle")
	{
		...
		$pdf->MultiCell($w, 3, "Bon de livraison", '', 'L');
		...
		if($pdf->PageNo()==1){
			...
			$pdf->SetXY($posx, $posy);
			$pdf->SetTextColor(0, 0, 60);
			$pdf->MultiCell($w, 3, "Date : ".dol_print_date($object->date, "day", false, $outputlangs, true), '', 'L');
		}
