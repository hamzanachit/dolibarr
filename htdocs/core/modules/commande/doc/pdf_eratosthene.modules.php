			...
			if (file_exists($dir)) {
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($this->getColumnStatus('subprice')) {
						...
						$up_excl_tax_cleaned = str_replace(' ', '', $up_excl_tax);

						// Ensure the value is cast to a float, round it, and then format it
						$rounded_up_excl_tax = round((float)$up_excl_tax_cleaned, 2);
						$formatted_up_excl_tax = number_format($rounded_up_excl_tax, 2, ',', '');

						// Output the formatted value in the PDF
						$this->printStdColumnContent($pdf, $curY, 'subprice', $formatted_up_excl_tax);
						$nexY = max($pdf->GetY(), $nexY);
					}
					...
					// Total excl tax line (HT)
						...
						// Retrieve the total excluding tax (HT)
					$total_excl_tax = pdf_getlinetotalexcltax($object, $i, $outputlangs, $hidedetails);

					// Debugging: Check the raw value returned
					var_dump($total_excl_tax); // This will output the raw value for debugging purposes

					// Remove any spaces and commas from the total excluding tax value
					$total_excl_tax_cleaned = str_replace([' ', ','], ['', '.'], $total_excl_tax);

					// Convert the cleaned string to a float
					$total_excl_tax_float = (float)$total_excl_tax_cleaned;

					// Debugging: Check the float value after cleaning and conversion
					var_dump($total_excl_tax_float); // To verify the converted float value

					// Manually add 20% tax to the total excluding tax
					$total_incl_tax = $total_excl_tax_float * 1.20; // Add 20% tax

					// Format the total including tax for display (2 decimals, comma as decimal separator)
					$formatted_total_incl_tax = number_format($total_incl_tax, 2, ',', '');

					// Output the formatted total including tax in the PDF
					$this->printStdColumnContent($pdf, $curY, 'totalexcltax', $formatted_total_incl_tax);
...
protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
{
    ...
    $footerText .= "RC: 397973 - Patente: 31990804 - CNSS: 5880275 - IF: 25093040 - ICE: 002044867000021";
