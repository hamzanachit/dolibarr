...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_bricolocalentete.modules.php
 *	\ingroup    facture
 *	\brief      File of class to generate customers invoices from bricolocalentete model
 */
...
/**
 *	Class to generate the customer invoice PDF with template bricolocalentete
 */
class pdf_bricolocalentete extends ModelePDFFactures
{
	 /**
	...
	public function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "bricolocalentete";
		$this->description = $langs->trans('PDFbricolocalenteteDescription');
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->facture->dir_output) {
			...
			if (file_exists($dir)) {
				...
				//$heightforfooter = $this->marge_basse + (empty($conf->global->MAIN_GENERATE_DOCUMENTS_SHOW_FOOT_DETAILS) ? 22 : 22); // Height reserved to output the footer (value include bottom margin)
						...
						if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) {
							...
							if ($this->emetteur->logo) {
								...
								if (empty($conf->global->MAIN_PDF_USE_LARGE_LOGO)) {
									$logo = $logodir . '/logos/background.png' ;
								} else {
									$logo = $logodir . '/logos/background.png' ;
								}
								if (is_readable($logo)) {
									//$height = pdf_getHeightForLogo($logo);
									$pdf->Image($logo, $this->marge_gauche-4, $posy-3, 0, $height); // width=0 (auto)
								} else {
                            ...
                            $pdf->SetXY($posx, $posy);
                            $pdf->MultiCell($w, 4,  " Page" . $pagenb.'/'.$pageposafter , '', 'R');
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null)
	{
		...
		if (!empty($conf->global->PDF_SHOW_PROJECT_TITLE)) {
			...
			if (!empty($object->project->ref)) {
				...
				$pdf->SetXY($posx, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell($w, 3, $outputlangs->transnoentities("Project") . " : " . (empty($object->project->title) ? '' : $object->projet->title), '', 'L');
			}
		}
		...
		if (!empty($conf->global->PDF_SHOW_PROJECT)) {
			...
			if (!empty($object->project->ref)) {
				...
				$pdf->SetXY($posx, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell($w, 3, $outputlangs->transnoentities("RefProject") . " : " . (empty($object->project->ref) ? '' : $object->projet->ref), '', 'L');
			}
		}
		...
		$pdf->MultiCell($w, 3, $title . " : " . dol_print_date($object->date, "day", false, $outputlangs), '', 'L');
		...
		if ($object->type != 2) {
			...
			$pdf->MultiCell($w, 3, $title." : ".dol_print_date($object->date_lim_reglement, "day", false, $outputlangs, true), '', 'L');
		}
		/*
		...
		$current_y = $pdf->getY();
		$posy = pdf_writeLinkedObjects($pdf, $object, $outputlangs, $posx, $posy, $w, 3, 'L', $default_font_size+1);
