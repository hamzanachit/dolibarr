 ...
 * Copyright (C) 2017-2018		Bigenwald Mathieu		<matsky8080@@hotmail.com>
 *
...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_bricorefpro.modules.php
 *	\ingroup    facture
 ...
 */
class pdf_x4rComplet extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
    ...
    var $phpmin = array(5,3,0); // Minimum version of PHP required by module
    var $version = '1.1';
	...
	{
		global $conf,$langs,$mysoc,$show_bl_ref,$blref;
		$this->show_bl_ref = 1;
		$this->blref='';
		$langs->load("main");
		$langs->load("bills");
  		$langs->load("bricorefpro");

		$this->db = $db;
		$this->name = "x4rComplet";
		$this->description = $langs->trans('PDFbricorefproDescription');
		...
		// Define position of columns
 		 //Element 1 origine colonne desc
		if (!empty($this->show_bl_ref))
		{
			$this->posxdesc=$this->marge_gauche+24;
  		}else{
			$this->posxdesc=$this->marge_gauche+1;
		  }
		if($conf->global->PRODUCT_USE_UNITS)
		{
			$this->posxblref=$this->marge_gauche+1;
			$this->posxtva=79;
			$this->posxup=94;
			$this->posxqty=110;
			$this->posxunit=127;
		...
		$this->posxdiscount=142;
		$this->posxprogress=106; // Only displayed for situation invoices
		$this->postotalht=154;
 	 	$this->posxttc=176;

  		//Element 2 ajout taille colonne ttc
	...
	function write_file($object,$outputlangs,$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if (! empty($conf->global->MAIN_USE_FPDF)) $outputlangs->charset_output='ISO-8859-1';//UTF-8//ISO-8859-1
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				if (empty($this->atleastonediscount) && empty($conf->global->PRODUCT_USE_UNITS))    // retreive space not used by discount
				{
					...
					$this->postotalht+=($this->posxttc - $this->posxttc); //posxtotalht???
    				 $this->posxttc;
				}
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					{
						if(!$i==0){
							$bl_prec = pdf_getlinebl_ref($object, $i-1, $outputlangs, $hidedetails);
						}
							$blref = pdf_getlinebl_ref($object, $i, $outputlangs, $hidedetails);

						if($bl_prec != $blref){
							
							$pdf->SetXY($this->posxpicture-$curX-$progress_width, $curY);
							//tawil
							$pdf->SetFont('','B', $default_font_size -1);
							$pdf->MultiCell($this->posxpicture-$curX-$progress_width,3, $blref,'','L');
							$curY+=3;
						}

					}
					$pdf->SetFont('','', $default_font_size -1);
					// pdf_writelinedesc($pdf,$object,$i,$outputlangs,$this->posxpicture-$curX-$progress_width+15,3,$curX,$curY,$hideref,$hidedesc);



					// Fetch or set the line description
	$line_description = $object,$i,$outputlangs;  // Hypothetical function

	// Assume manipulation is required on $line_description
	if (strpos($line_description, "commande") !== false) {
		// Example: remove 'commande' from description or process accordingly
		$line_description = str_replace("commande", "", $line_description);
}

// Now call the function to write this description to the PDF
pdf_writelinedesc(
    $pdf,
    $object,
    $i,
    $outputlangs,
    $this->posxpicture - $curX - $progress_width + 15,  // calculated width
    3,  // height of the cell
    $curX,  // X position
    $curY,  // Y position
    $hideref,
    $hidedesc
					...
					{
						//if(!$i==0){
							//$bl_prec = pdf_getproduct_ref($object, $i-1, $outputlangs, $hidedetails);
					//	}
							$blref = pdf_getproduct_ref($object, $i, $outputlangs, $hidedetails);

					//	if($bl_prec != $blref){
							
							$pdf->SetXY($this->posxblref-1, $curY);
							//tawil
							$pdf->MultiCell(24,2, $blref,'','L');
					//	}
                ...
                // Debugging output to see the value before any manipulation
                // var_dump("Unit Price Excl Tax before formatting: " . $up_excl_tax);
                
                // Replace comma with a dot if the original format uses a comma as decimal separator
                $up_excl_tax = str_replace(',', '.', $up_excl_tax);
                
                // Convert the string to a float to ensure calculations are correct
                $up_excl_tax = floatval($up_excl_tax);
                
                // Format the number to two decimal places and use a comma as the decimal separator
                $formatted_up_excl_tax = number_format($up_excl_tax, 2, ',', '');
                
                // Debugging output to check the formatted value
                // var_dump("Unit Price Excl Tax after formatting: " . $formatted_up_excl_tax);
                
                // Set the position on the PDF and print the formatted unit price
                $pdf->SetXY($this->posxup, $curY);
                $pdf->MultiCell($this->posxqty-$this->posxup-0.8, 3, $formatted_up_excl_tax, 0, 'R', 0);
				...
				// Total HT line
                $total_excl_tax = pdf_getlineupwithtax($object, $i, $outputlangs, $hidedetails);
                $total_excl_tax = str_replace(',', '.', $total_excl_tax);
                // Convert the adjusted string to a float
                $total_excl_tax = floatval($total_excl_tax);
                // Format the number to two decimal places and use a comma as the decimal separator
                $formatted_total_excl_tax = number_format($total_excl_tax, 2, ',', '');
                
                // Use var_dump to output the formatted value
                // var_dump("Total Excl Tax after formatting: " . $formatted_total_excl_tax);
                
                $pdf->SetXY($this->postotalht, $curY);
                $pdf->MultiCell($this->posxttc-$this->postotalht, 3, $formatted_total_excl_tax, 0, 'R');
					...
					//$pdf->MultiCell($this->posxqty-$this->posxup-0.8, 3, $up_excl_tax, 0, 'R', 0);
     				...
     				//Element 4 Ajout colonne TTC par calcul sur $total_excl_tax par $vat_rate
					$total_incl_tax = price($object->lines[$i]->total_ht+$object->lines[$i]->total_tva );
     				$pdf->SetXY($this->posxttc, $curY);
					$pdf->MultiCell($this->page_largeur-$this->marge_droite-$this->posxttc, 3, $total_incl_tax, 0, 'R', 0);
					...
					while ($pagenb < $pageposafter)
					{
						...
						{
							$this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforfooter, 0, $outputlangs, 1, 1, $object->multicurrency_code,$pagenb);
						}
				...
				{
					$this->_tableau($pdf, $tab_top_newpage, $this->page_hauteur - $tab_top_newpage - $heightforinfotot - $heightforfreetext - $heightforfooter , 0, $outputlangs, 1, 0, $object->multicurrency_code,$pagenb);
					$bottomlasttab=$this->page_hauteur - $heightforinfotot - $heightforfreetext - $heightforfooter + 1;
				}
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($object->type != 2)
		{
			...
			//$//test = convertToWord(round($object->total_ttc,2),$langs);
			//$test = 
			//var_dump($test);
			
			$pdf->SetXY($this->marge_gauche, $posy+2);
			$pdf->SetFont('', 'B', 8);
			$str=round($object->total_ttc,2);
			$lettre=nombreEnLettres($str);
			//	var_dump($str);
			//var_dump($lettre);
			//ucwords($outputlangs->getLabelFromNumber(round($object->total_ttc,2),1,$conf->monnaie))
			$pdf->MultiCell(100, 3,"Arrêté la présente facture à la somme de :".$lettre, 0, 'L', 0);
	 ...
	 *   @return	void
	 */
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='',$nbpages= NULL)
	{
		global $conf;
		//var_dump($pagenb);die;
		...
		$titre = "Page " . $pdf->PageNo().'/'.$pdf->getAliasNbPages();
		...
		if (!empty($this->show_bl_ref))
		{
			$pdf->line($this->posxblref+22, $tab_top, $this->posxblref+22, $tab_top + $tab_height);

				// line prend une position y en 2eme param et 4eme param
			$pdf->SetXY($this->posxblref, $tab_top+1);
			$pdf->MultiCell(111,2, 'Ref','','L');
		...
		if ($this->atleastonediscount)
		{
			...
			$pdf->line($this->posxttc, $tab_top, $this->posxttc, $tab_top + $tab_height);
			$pdf->SetXY($this->postotalht-1, $tab_top+1);
			$pdf->MultiCell(22,2, $outputlangs->transnoentities("P.U TTC"),'','C');
			$pdf->SetXY($this->posxttc-1, $tab_top+1);
			$pdf->MultiCell(25,2, $outputlangs->transnoentities("TotalTTC"),'','R');
		//}
	}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $outputlangsbis = null, $titlekey = "PdfOrderTitle")
	{
		...
		$outputlangs->loadLangs(array("main", "bills", "propal", "facture", "companies"));
...
protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
{
    ...
    // Define your custom footer text
    $footerText = "34 BIS, RUE 8 RDC - ANTARIA3 - Hay Mohammadi CASABLANCA - T/Fax: 05 22 66 2992 - Email: lachgar1974@gmail.com\n";
    $footerText .= "RC: 397973 - Patente: 37952638 - CNSS: 5880275 - IF: 25093040 - ICE: 002044867000021";

    // Add the footer text to the PDF at the bottom of the page
    $pdf->SetY(-20);  // Adjust Y position to move the text to the bottom
    $pdf->SetFont('', 'I', 8);  // Set font style and size
    $pdf->MultiCell(0, 3, $footerText, 0, 'C');
