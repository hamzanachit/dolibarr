...
class pdf_x4rComplet extends ModelePDFFactures
{
	...
	function __construct($db)
	{
		...
		$this->marge_haute =isset($conf->global->MAIN_PDF_MARGIN_TOP)?$conf->global->MAIN_PDF_MARGIN_TOP:15;
		...
		if ($conf->facture->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblignes; $i++)
				{
					...
					//var_dump($total_excl_tax);die;
					$pdf->SetXY($this->postotalht, $curY);
					$pdf->MultiCell($this->posxttc-$this->postotalht, 3, $total_excl_tax, 0, 'R');
	...
	function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($object->type != 2)
		{
			...
			$pdf->SetFont('', 'B', 8);
			$pdf->MultiCell(100, 3,"Arrêté la présente facture à la somme de :" . ucwords($outputlangs->getLabelFromNumber(round($object->total_ttc,2),1,$conf->monnaie)), 1, 'L', 0);
			$posy = $pdf->GetY() + 1;
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($showaddress)
		{
			...
			$ice = $thirdparty = $object->thirdparty->idprof5;
            $yearned_ice=dol_print_date($object->date,'%Y');

            if($ice){
                $posy = $pdf->getY();
                $pdf->SetFont('','', $default_font_size - 1);
                $pdf->SetXY($posx+2,$posy);
                $pdf->MultiCell($widthrecbox, 4, "ICE :" . $ice, 0, 'L');

            }
            elseif($yearned_ice>="2019"){
                $posy = $pdf->getY();
                $pdf->SetFont('','B', $default_font_size + 2);
                $pdf->SetTextColor(255,0,0);
                $pdf->SetXY($posx+2,$posy);
                $pdf->MultiCell($widthrecbox, 4, "ATTENTION MANQUE D'ICE CLEINT!!" , 0, 'L');
            }
	...
	function _pagefoot(&$pdf,$object,$outputlangs,$hidefreetext=0)
	{
		...
		return pdf_pagefoot($pdf,$outputlangs,'INVOICE_FREE_TEXT',$this->emetteur,$this->marge_basse,$this->marge_gauche,$this->marge_haute,$this->page_hauteur,$object,$showdetails,$hidefreetext);
	}
