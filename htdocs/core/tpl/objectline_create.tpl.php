...
<tr
    ...
    <td class="nobottom linecoldescription minwidth400imp">
		...
		if (empty($conf->global->MAIN_DISABLE_FREE_LINES)) {
			...
			$defaultType = GETPOSTISSET("type") ? GETPOST("type", 'alpha', 2) : 0; // 0 is for a predefined product line, adjust as needed
			$form->select_type_of_lines($defaultType, 'type', 1, 1, $forceall);
			echo '</span>';
		}
		if (isModEnabled("product") || isModEnabled("service")) {
...
if (event.which != 9 && (event.which < 37 ||event.which> 40) && jQuery("#price_ht").val() != '') {
	...
	if (in_array($this->table_element_line, array('propaldet', 'commandedet', 'facturedet'))) { ?>
		...
		if (empty($conf->global->MAIN_DISABLE_EDIT_PREDEF_PRICEHT) && empty($senderissupplier)) {
            ...
            console.log("Load unit price and set it into #price_ht or #price_ttc for product id="+$(this).val()+"
            ...
            function(data) {
            console.log("objectline_create.tpl Load unit price ends, we got value ht="+data.price_ht+"
            ...
            <?php print getDolGlobalInt('MAIN_NO_INPUT_PRICE_WITH_TAX') ? 'false' : 'true'; ?>) {
            ...
            <?php
							...
							if (getDolGlobalString('MAIN_SALETAX_AUTOSWITCH_I_CS_FOR_INDIA')) {
								?>
            console.log("MAIN_SALETAX_AUTOSWITCH_I_CS_FOR_INDIA is on so we check if we need to autoswith the vat
            code");
            console.log("mysoc->country_code=<?php echo $mysoc->country_code; ?>
            thirdparty->country_code=<?php echo $object->thirdparty->country_code; ?>");
            ...
            <?php
								if ($mysoc->country_code == 'IN' && !empty($object->thirdparty) && $object->thirdparty->country_code == 'IN' && $mysoc->state_code == $object->thirdparty->state_code) {
									...
									?>
            console.log("Countries are both IN and states are same, so we revert I into CS in
            ...
            <?php
									...
									?>
            console.log("Countries are both IN and states differs, so we revert CS into I in
            default_vat_code="+default_vat_code);
            new_default_vat_code = default_vat_code.replace(/^C\+S\-/, 'I-');
            <?php
								}
								?>
            if (new_default_vat_code != default_vat_code && jQuery('#tva_tx
            option:contains("'+new_default_vat_code+'")').val()) {
		...
		if (!empty($usemargins) && $user->rights->margins->creer) {
            ...
            options += '<option value="inputprice" price="'+defaultprice+'">
                <?php echo dol_escape_js($langs->trans("InputPrice").'...'); ?></option>';
		...
		?> // Deal with supplier ref price (idprodfournprice=int) if (jQuery('#idprodfournprice').val()> 0)
                {
                console.log("objectline_create.tpl #idprodfournprice is an ID > 0, so we set some properties into
                page");
                ...
                var supplier_ref = $('option:selected', this).attr('data-supplier-ref'); // When select is done from
                HTML select
                if (typeof supplier_ref === 'undefined') { supplier_ref =
                jQuery('#idprodfournprice').attr('data-supplier-ref');} // When select is done from HTML input with ajax
                autocomplete

                console.log("objectline_create.tpl We find supplier price : up = "+up+", up_locale = "+up_locale+",
                supplier_ref = "+supplier_ref+" qty = "+qty+", tva_tx = "+tva_tx+", default_vat_code =
                "+default_vat_code+", stringforvatrateselection="+stringforvatrateselection+", discount = "+discount+"
                for product supplier ref id = "+jQuery('#idprodfournprice').val());
			...
			if (getDolGlobalInt('PRODUIT_AUTOFILL_DESC') == 1) {
				?> var description=$('option:selected', this).attr('data-description'); if (typeof description=='undefined' ) {
                    description=jQuery('#idprodfournprice').attr('data-description'); } console.log("Load desciption
                    into text area : "+description);
				<?php
				if (!empty($conf->global->FCKEDITOR_ENABLE_DETAILS)) {
					?>
			if (typeof CKEDITOR == " object" && typeof CKEDITOR.instances !="undefined" ) { var
                    editor=CKEDITOR.instances['dp_desc']; if (editor) { editor.setData(description); } } <?php
				} else {
