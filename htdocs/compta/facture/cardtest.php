...
if (empty($reshook)) {
	...
	if ($action == 'confirm_clone' && $confirm == 'yes' && $permissiontoadd) {
		...
		if ($canconvert) {
			...
			//
			//var_dump($amount_ht);var_dump($amount_tva);var_dump($amount_ttc);die;
							...
							if ($result > 0) {
								...
								for ($i = 0; $i < $num; $i++) {
									...
									if ($lines[$i]->subprice < 0 && empty($conf->global->INVOICE_KEEP_DISCOUNT_LINES_AS_IN_ORIGIN)) {
										...
										/*solution 1
										$cmd = new Commande($db);
										$cmd->fetch($object->linked_objects);
										$bl_ref=$cmd->ref;
										var_dump($bl_ref);die;
										*/

										$cmd = new Commande($db);
										$cmd->fetch($object->linked_objects['commande']);
										$bl_ref=$cmd->ref;
										//var_dump($bl_ref);die;
		...
		if (!$error && ($qty >= 0) && (!empty($product_desc) || (!empty($idprod) && $idprod > 0))) {
			...
			if (GETPOST('price_ht') != '' || GETPOST('price_ht_devise') != '') {
				$pu_ht = price2num($price_ht, 'MU',2); // $pu_ht must be rounded according to settings
			} else {
				$pu_ttc = price2num(GETPOST('price_ttc'), 'MU',2);
				$pu_ht = price2num($pu_ttc / (1 + ($tva_tx / 100)), '',2); // $pu_ht must be rounded according to settings
			}
			...
			$price2num_pu_ht = price2num($pu_ht,'',2);
			...
			if ($usercanproductignorepricemin && (!empty($price_min) && ($price2num_pu_ht * (1 - $price2num_remise_percent / 100) < $price2num_price_min))) {
				...
				// Insert line
				$result = $object->addline($bl_ref,$desc, $pu_ht, $qty, $tva_tx, $localtax1_tx, $localtax2_tx, $idprod, $remise_percent, $date_start, $date_end, 0, $info_bits, '', $price_base_type, $pu_ttc, $type, - 1, $special_code, '', 0, GETPOST('fk_parent_line'), $fournprice, $buyingprice, $label, $array_options, $_POST['progress'], '', $fk_unit, $pu_ht_devise);
		...
		if ((GETPOST('price_ht') != '' || GETPOST('price_ht_devise') != '') && GETPOST('price_ttc')=='') {
			$pu_ht = price2num(GETPOST('price_ht'), 'MU'); // $pu_ht must be rounded according to settings
			$price_base='HT';

		} else {
			$pu_ht = price2num(GETPOST('price_ttc'), 'MU');
			$price_base='TTC';
...
if ($action == 'create') {
	...
	if ($soc->id > 0 && (!GETPOST('fac_rec', 'int') || !empty($invoice_predefined->frequency))) {
		...
		if ($soc->outstanding_limit != '') {
			if ($outstandingBills > $soc->outstanding_limit) {
				?><span style="color:red; font-weight:bold;font-size:15px"><?php print price($outstandingBills, '', $langs, 0, 0, -1, $conf->currency);?></span><?php
	...
	// Button "Create Draft"
	if (!$soc->array_options['options_actv']==1) {
		...
		print '</div>';
			...
			print '<div class="center">';
			print "<script> document.getElementById('btnvalidform').addEventListener('click', function(event){
				event.preventDefault()
			  });</script>";
			print '<input type="submit" id="btnvalidform" class="button" name="bouton" value="'.$langs->trans('CreateDraft').'"disabled>';
			print '<input type="button" class="button button-cancel" value="'.$langs->trans("Cancel").'" onClick="javascript:history.go(-1)">';
			print '</div>';
		}
